...
const systemHealth = [
  ...
  { name: 'PostgreSQL DB', status: 'Operational', color: '#22C55E' },
  { name: 'JWT Auth', status: 'Operational', color: '#22C55E' },
  { name: 'Audit Logger', status: 'Operational', color: '#22C55E' },
  { name: 'System Uptime', status: '99.98%', color: '#22C55E' },
]
...
export default function AdminDashboard({ user }: Props) {
  ...
  const weeklyData = [
    { day: 'Mon', present: Math.max(0, todayBreakdown.present - 4), absent: Math.max(0, todayBreakdown.absent - 2), late: todayBreakdown.late, leave: todayBreakdown.onLeave },
    { day: 'Tue', present: Math.max(0, todayBreakdown.present - 2), absent: todayBreakdown.absent, late: todayBreakdown.late, leave: todayBreakdown.onLeave },
    { day: 'Wed', present: todayBreakdown.present, absent: todayBreakdown.absent, late: todayBreakdown.late, leave: todayBreakdown.onLeave },
    { day: 'Thu', present: todayBreakdown.present, absent: todayBreakdown.absent, late: todayBreakdown.late, leave: todayBreakdown.onLeave },
    { day: 'Fri', present: todayBreakdown.present, absent: todayBreakdown.absent, late: todayBreakdown.late, leave: todayBreakdown.onLeave },
  ]

  useEffect(() => {
    analyticsApi.getExecutiveDashboard().then(res => {
      if (res) {
        if (res.overview) setOverview(res.overview)
        if (res.todayBreakdown) setTodayBreakdown(res.todayBreakdown)
        if (res.branchStats && res.branchStats.length > 0) setBranches(res.branchStats)
        if (res.deptStats && res.deptStats.length > 0) setDepts(res.deptStats)
        if (res.auditLogs && res.auditLogs.length > 0) {
          setLogs(res.auditLogs.map((l: any) => ({
            user: l.user_name || 'System',
            action: l.action,
            time: l.created_at ? l.created_at.slice(11, 16) : 'Recent',
            type: l.log_type === 'success' ? 'success' : (l.log_type === 'warning' ? 'warning' : 'info')
          })))
        }
      }
    }).catch(() => {})
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
        {[
          { label: 'Total Employees', value: overview.totalEmployees, color: '#FFFFFF' },
          { label: 'Active Today', value: overview.activeToday, color: '#22C55E' },
          { label: 'HR Managers', value: overview.hrManagers, color: '#3B82F6' },
          { label: 'Branches', value: overview.branches, color: '#D4AF37' },
          { label: 'Departments', value: overview.departments, color: '#A855F7' },
        ].map(s => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="rounded-2xl p-5" style={{ background: '#111111', border: '1px solid #2A2A2A' }}>
          ...
          <div className="space-y-3">
            {branches.length === 0 ? (
              <div className="text-xs text-gray-500 py-4 text-center">Loading branch performance...</div>
            ) : (
              branches.map(b => (
        ...
        <div className="rounded-2xl p-5" style={{ background: '#111111', border: '1px solid #2A2A2A' }}>
          ...
          <div className="space-y-3">
            {depts.length === 0 ? (
              <div className="text-xs text-gray-500 py-4 text-center">Loading department stats...</div>
            ) : (
              depts.map(d => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 rounded-2xl p-5" style={{ background: '#111111', border: '1px solid #2A2A2A' }}>
          ...
          <div className="space-y-3">
            {logs.length === 0 ? (
              <div className="text-xs text-gray-500 py-4 text-center">No recent audit activity.</div>
            ) : (
              logs.map((log, i) => (
