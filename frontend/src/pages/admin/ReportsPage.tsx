...
export default function ReportsPage({ role }: Props) {
  ...
  const [reports, setReports] = useState<any[]>([
    { name: 'Monthly Attendance Summary — August 2026', type: 'Attendance', generated: '2026-08-30', size: '245 KB' },
    { name: 'Employee Leave Analytics — Q3 2026', type: 'Leave', generated: '2026-08-30', size: '128 KB' },
    { name: 'Department Headcount & Performance', type: 'Analytics', generated: '2026-08-30', size: '512 KB' },
  ])
  ...
  useEffect(() => {
    analyticsApi.getDepartments().then(res => {
      ...
      if (Array.isArray(list) && list.length > 0) {
        setDeptData(list.map((d: any) => ({
          dept: d.code || d.name.slice(0, 4),
  ...
  const handleDownloadCSV = (reportName: string) => {
    const csvContent = "data:text/csv;charset=utf-8,Report Name,Generated Date,Status\\n" +
      `${reportName},${new Date().toISOString().slice(0, 10)},Verified\\n`
