...
export default function AttendanceCalendarPage() {
  ...
  const summary = {
    present: Object.entries(recordsMap).filter(([k, v]) => k.startsWith(`${year}-${String(month+1).padStart(2,'0')}`) && v === 'Present').length,
    late: Object.entries(recordsMap).filter(([k, v]) => k.startsWith(`${year}-${String(month+1).padStart(2,'0')}`) && v === 'Late').length,
    absent: Object.entries(recordsMap).filter(([k, v]) => k.startsWith(`${year}-${String(month+1).padStart(2,'0')}`) && v === 'Absent').length,
    leave: Object.entries(recordsMap).filter(([k, v]) => k.startsWith(`${year}-${String(month+1).padStart(2,'0')}`) && v === 'Leave').length,
  }
